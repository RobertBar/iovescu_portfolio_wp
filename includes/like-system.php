...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( 'Like', 'iovescu-portfolio' ),
		'unlike' => __( 'Unlike', 'iovescu-portfolio' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'iovescu-portfolio' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'iovescu-portfolio' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'iovescu-portfolio' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = __( '', 'iovescu-portfolio' );
	if ( is_numeric( $like_count ) && $like_count > 0 ) { 
		$number = sl_format_count( $like_count );
	} else {
		$number = $like_text;
	}
	$count = '<span class="sl-count" style="padding-top: 3px;">' . $number . '</span>';
	return $count;
} // get_like_count()
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( '', 'iovescu-portfolio' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'iovescu-portfolio' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
